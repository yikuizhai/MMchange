...
# 读取json文件
with open('/media/WD_2T/ZYJ/315_cd_data/LEVIR_256/256/total1.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
...
robert_tokenizer = RobertaTokenizer.from_pretrained('/media/WD_2T/ZYJ/duomotai_test/roberta-base')
robert_model = RobertaModel.from_pretrained('/media/WD_2T/ZYJ/duomotai_test/roberta-base')
...
def val(args, val_loader, model, epoch):
    global text_A_features_list
    global text_B_features_list
    model.eval()
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        img, target, pre_image_name, post_image_name = batched_inputs
        pre_img = img[:, 0:3]
        post_img = img[:, 3:6]
        output_text_A = [dictionary.get(img_name) for img_name in pre_image_name]
        output_text_B = [dictionary.get(img_name) for img_name in post_image_name]
        text_A = clip.tokenize([out_A for out_A in output_text_A]).to(device)
        text_B = clip.tokenize([out_B for out_B in output_text_B]).to(device)
        text_A_features = model_text_decode.encode_text(text_A)
        text_B_features = model_text_decode.encode_text(text_B)
        # text_A_features_list = []
        # for out_A in output_text_A:
        #     text_A = robert_tokenizer(out_A, return_tensors='pt')
        #     text_A_features_list.append(robert_model(**text_A).get('pooler_output'))
        # text_B_features_list = []
        # for out_B in output_text_B:
        #     text_B = robert_tokenizer(out_B, return_tensors='pt')
        #     text_B_features_list.append(robert_model(**text_B).get('pooler_output'))
        # text_A = robert_tokenizer([out_A for out_A in output_text_A],return_tensors='pt')
        # text_B = robert_tokenizer([out_B for out_B in output_text_B],return_tensors='pt')
        # text_A_features = torch.cat(text_A_features_list, dim=0).cuda()
        # text_B_features = torch.cat(text_B_features_list, dim=0).cuda()
        ...
        if args.onGPU == True:
            ...
            text_A_features = text_A_features.float().cuda()
            text_B_features = text_B_features.float().cuda()
        pre_img_var = torch.autograd.Variable(pre_img).float()
        post_img_var = torch.autograd.Variable(post_img).float()
        target_var = torch.autograd.Variable(target).float()

        # run the mdoel
        output, output2, output3, output4 = model(pre_img_var, post_img_var, text_A_features, text_B_features)
...
def trainValidateSegmentation(args):
    ...
    if args.file_root == 'LEVIR':
        args.file_root = '/media/WD_2T/ZYJ/315_cd_data/LEVIR_256/256（复件）'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', default='./results16', help='Directory to save the results')
